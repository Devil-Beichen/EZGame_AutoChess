...
AGridMap::AGridMap()
{
	...
	Mesh = CreateDefaultSubobject<UProceduralMeshComponent>(TEXT("MapMesh"));
...
void AGridMap::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	//当行、列、大小改变时重新生成棋盘
	if (PropertyChangedEvent.Property->GetName() == "Roll" || PropertyChangedEvent.Property->GetName() == "Colume" ||
...
void AGridMap::GenerateHexNodes(float InHexSize, int InRoll, int InColume)
{
	...
	for (int i = 0; i < InRoll; ++i)
	{
		for (int j = 0; j < InColume; ++j)
		{
			...
			tHexLocation.Y = i % 2 == 0
				                 ? (FMath::Sqrt(3) * InHexSize * j)
				                 : (FMath::Sqrt(3) * InHexSize * j + FMath::Sqrt(3) * 0.5 * InHexSize);
			tHexLocation.Z = 0;
...
bool AGridMap::FindPath(TArray<UGridNode*>& Path, AActor* InActor, UGridNode* FromNode, UGridNode* ToNode,
                        int StopSteps)
{
	Path.Empty();
	//安全判断
...
void AGridMap::GenerateMapMesh()
{
	...
	for (auto a : NodeMap)
	{
		...
		//生成索引区块模型
		Mesh->CreateMeshSection(Index, Vertices, Indecies, Normals, UV1, EmptyArray, EmptyArray, EmptyArray,
		                        VertexColors, MeshTangents, false);
		//设置材质
...
//判断是否在六边形内
//https://blog.csdn.net/iteye.10018/article/details/82541460
